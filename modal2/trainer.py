...
class DHGNNTrainer:
    ...
    def fit(self):
        ...
        # Use custom init_edge_features if provided, otherwise default to modality_tensors
        if self.init_edge_features is not None:
            init_edge_features = [torch.FloatTensor(f).to(self.device) for f in self.init_edge_features]
        else:
        ...
        for epoch in range(self.epochs):
            ...
            if (epoch + 1) % 5 == 0 or epoch == 0:
                ...
                if (epoch + 1) % 20 == 0 or epoch == 0:
                    ...
                    mi_msg = ""
                    mi_c = metrics.get("morans_i_cluster", None)
                    if mi_c is not None:
                        mi_msg = f" | MI_c {mi_c:.3f}"
                    print(
        ...
        # 打印 Moran's I（无论有无标签均可用）
        mi_cluster = metrics.get("morans_i_cluster", None)
        mi_emb = metrics.get("morans_i_embedding_mean", None)
        if mi_cluster is not None:
            print(f"Moran's I (cluster labels): {mi_cluster:.4f}")
            print(f"Moran's I (embedding mean): {mi_emb:.4f}")
    ...
    def _evaluate(self, model, modality_tensors):
        ...
        # Moran's I: 无需标签，基于空间坐标评估聚类的空间连续性
        try:
            # 对聚类标签计算 Moran's I（聚类标签编码为整数）
            morans_cluster = compute_morans_i(self.coords, predictions.astype(float), k=10)
            metrics["morans_i_cluster"] = morans_cluster

            # 对 embedding 各维度计算 Moran's I 并取均值
            n_dims = min(embedding.shape[1], 20)  # 最多取前20维避免计算过慢
            morans_emb_list = []
            for d in range(n_dims):
                mi = compute_morans_i(self.coords, embedding[:, d], k=10)
                morans_emb_list.append(mi)
            metrics["morans_i_embedding_mean"] = float(np.mean(morans_emb_list))
        except Exception as e:
            metrics["morans_i_cluster"] = float("nan")
            metrics["morans_i_embedding_mean"] = float("nan")

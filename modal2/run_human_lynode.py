...
def parse_args():
    ...
    p.add_argument("--n_pca_rna", type=int, default=30)
    ...
    p.add_argument("--freeze_edges_after_warmup", action=argparse.BooleanOptionalAction, default=False)
...
def main():
    ...
    rna_features = pca(adata_rna_high, n_comps=args.n_pca_rna, random_state=args.seed)
    ...
    prot_features = pca(adata_prot, n_comps=n_prot_pca, random_state=args.seed)
    ...
    if label_col in adata_rna.obs.columns:
        ...
        labels = None
        n_classes = 10  # default estimate for human lymph node

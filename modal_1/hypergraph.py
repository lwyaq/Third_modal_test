...
def gene_as_hyperedge(
    X_gene: np.ndarray,
    min_cells_pct: float = 0.05,
    max_cells_pct: float = 0.40,
    max_genes: int = 500,
    use_expression_weights: bool = True,
) -> Tuple[sp.csr_matrix, int]:
    """
    Build hypergraph using the "gene-as-hyperedge" paradigm from
    He et al. (Bioinformatics, 2026).

    Core idea: the scRNA-seq expression matrix IS the hypergraph
    incidence matrix.
      - Nodes = cells (rows of X_gene)
      - Hyperedges = genes (columns of X_gene)
      - H(v, e) = expression of gene e in cell v (or binary)
      - Normalized per gene so each hyperedge has unit sum

    Parameters
    ----------
    X_gene      : (N_cells, N_genes) expression matrix.
    min_cells_pct : minimum fraction of cells expressing a gene
    max_cells_pct : maximum fraction of cells expressing a gene
    max_genes     : maximum number of gene hyperedges to keep
    use_expression_weights : if True, use normalized expression values
        as weights; if False, use binary (expressed/not expressed)

    Returns
    -------
    H_gene   : (N_cells, N_selected_genes) sparse incidence matrix
    n_genes  : number of selected gene hyperedges
    """
    if sp.issparse(X_gene):
        X = X_gene.toarray().astype(np.float32)
    else:
        X = X_gene.astype(np.float32)

    n_cells, n_genes_total = X.shape

    binary = (X > 0).astype(np.float32)
    gene_n_cells = binary.sum(axis=0)
    gene_pct = gene_n_cells / n_cells

    valid = (gene_pct >= min_cells_pct) & (gene_pct <= max_cells_pct)
    valid_idx = np.where(valid)[0]

    if len(valid_idx) == 0:
        raise ValueError(
            f"No genes pass filter (min={min_cells_pct:.1%}, max={max_cells_pct:.1%}). "
            "Try relaxing the thresholds."
        )

    if len(valid_idx) > max_genes:
        gene_var = X[:, valid_idx].var(axis=0)
        top_k = np.argsort(gene_var)[-max_genes:]
        valid_idx = valid_idx[np.sort(top_k)]

    X_sel = X[:, valid_idx]
    binary_sel = binary[:, valid_idx]

    if use_expression_weights:
        # Use actual expression values, normalized per gene
        gene_sums = X_sel.sum(axis=0, keepdims=True)
        gene_sums = np.maximum(gene_sums, 1.0)
        H_vals = X_sel / gene_sums
        # CRITICAL: enforce binary sparsity pattern
        # Division creates tiny nonzero values at originally-zero positions,
        # which sp.csr_matrix stores as nonzero entries → inflates nnz
        H_vals = H_vals * binary_sel
    else:
        # Binary incidence
        H_vals = binary_sel.copy()
        gene_sums = H_vals.sum(axis=0, keepdims=True)
        gene_sums = np.maximum(gene_sums, 1.0)
        H_vals = H_vals / gene_sums

    H_gene = sp.csr_matrix(H_vals.astype(np.float32))

    return H_gene, len(valid_idx)
